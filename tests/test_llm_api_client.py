...
class TestThinkingControl:
    """Thinking-mode control via the ``thinking`` client option."""

    @staticmethod
    def _body(api_url: str, thinking: bool = True) -> dict:
        client = LlmApiClient(
            model="gpt-4o",
            api_url=api_url,
            api_key="key",
        ...
        return client._build_request([{"role": "user", "content": "hi"}], [])

    def test_thinking_on_sends_no_thinking_field(self) -> None:
        body = self._body("https://openrouter.ai/api/v1", thinking=True)
        assert "reasoning" not in body
        assert "reasoning_effort" not in body

    def test_no_thinking_openrouter(self) -> None:
        body = self._body("https://openrouter.ai/api/v1", thinking=False)
        assert body["reasoning"] == {"effort": "none"}

    def test_no_thinking_ollama(self) -> None:
        body = self._body("http://localhost:11434/v1", thinking=False)
        assert body["reasoning_effort"] == "none"
        assert "reasoning" not in body

    def test_no_thinking_openai(self) -> None:
        body = self._body("https://api.openai.com/v1", thinking=False)
        assert body["reasoning_effort"] == "low"

    def test_no_thinking_unknown_provider_sends_nothing(self) -> None:
        body = self._body("https://llm.example.com/v1", thinking=False)
        assert "reasoning" not in body
        assert "reasoning_effort" not in body

    def test_no_thinking_still_attaches_tools(self) -> None:
        client = LlmApiClient(
            model="gpt-4o",
...
class TestProviderThinkingOffPayload:
    """Provider-aware extraction of the 'thinking off' request fields."""

    def test_openrouter(self) -> None:
        from llm_cli_py.providers.llm_api import provider_thinking_off_payload

        fields, supported = provider_thinking_off_payload("https://openrouter.ai/api/v1")
    ...
    def test_ollama(self) -> None:
        ...
        fields, supported = provider_thinking_off_payload("http://localhost:11434/v1")
        assert supported is True
    ...
    def test_openai(self) -> None:
        ...
        fields, supported = provider_thinking_off_payload("https://api.openai.com/v1")
        assert supported is True
    ...
    def test_unknown(self) -> None:
        ...
        fields, supported = provider_thinking_off_payload("https://llm.example.com/v1")
